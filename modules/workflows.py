...
# Stage 8: 主动学习 (active) 
# ==============================================================================
def run_stage_8_al_gpumd(sub_stage, data_path=None, work_dir=None,  dry_run=True, iteration=0):

    """
    Stage 8: GPUMD 主动学习 (迭代版)
    :param iteration: 迭代轮次，如果不为0，则使用 iter_XX/0X... 目录结构
    """
    ...
    # --- 路径计算逻辑 ---
    if iteration > 0:
        iter_root = f"iter_{iteration:02d}"
        path_train = os.path.join(iter_root, "01.train")
        path_md    = os.path.join(iter_root, "02.md")
        path_sel   = os.path.join(iter_root, "03.select")
        path_label = os.path.join(iter_root, "04.label")
    else:
        # 兼容性处理：如果不传 iter，依然使用带时间戳的旧模式
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        path_train = f"al_gpumd_train_{timestamp}"
        path_md    = f"al_gpumd_explore_{timestamp}"
        path_sel   = f"al_candidates_{timestamp}"
        path_label = f"workspace_al_dft_{timestamp}"

    print(f"\n=== Stage 8: GPUMD 主动学习 (Sub-Stage: 8.{sub_stage}) ===")
    if iteration > 0: print(f"[Workflow] 当前迭代轮次: {iter_root}")
    ...
    if sub_stage == 1:
        if not data_path:
            print("❌ 错误: 请指定 --data_path (包含 train.xyz)")
            return
        
        trainer = ALTrainer(path_train) # 自动创建 iter_01/01.train
    ...
    # ------------------------------------------------------------------
        ...
        # 如果没传 work_dir，自动推断上一步的目录
        if not work_dir and iteration > 0:
            work_dir = path_train
        
        if not work_dir or not os.path.exists(work_dir):
            print(f"❌ 错误: 找不到训练模型目录 {work_dir}")
            return
        if not data_path:
            print("❌ 错误: 请指定 --data_path (用于MD的model.xyz)")
            return

        model_dirs = sorted(glob.glob(os.path.join(work_dir, "model_*")))
        
        explorer = ALExplorer(path_md)
        explorer.prepare_exploration(model_dirs, data_path)
        print(f"\n✅ 探索目录就绪: {path_md}")

    # ------------------------------------------------------------------
    # 8.3 收集、筛选、生成 HONPAS 任务
    # ------------------------------------------------------------------
    elif sub_stage == 3:
        if not work_dir and iteration > 0:
            work_dir = os.path.join(f"iter_{iteration:02d}", "02.md")
        
        if not work_dir or not os.path.exists(work_dir):
            print(f"❌ 错误: 找不到探索目录 {work_dir}")
            return

        # 1. 筛选并保存 npy 到 03.select
        selector = ActiveSelector(work_dir)
        #  在这里存入 iter_01/03.select
        
        print(f"[Workflow] 正在执行筛选并备份至 {path_sel}...")
        # 修正：直接把筛选结果存到 iter 目录下，而不是 data/perturbed 下，方便管理
        candidates_sys = selector.select_and_save_direct(path_sel)

        if not candidates_sys: return

        # 2. 生成 HONPAS 任务到 04.label
        from modules.generation.wrapper import InputWrapper
        from modules.generation.scheduler import TaskScheduler
        
        template_path = cfg.TEMPLATE_MAP["scf"]
        wrapper = InputWrapper(template_path, cfg.SPECIES_MAP)
        # 任务生成在 iter_01/04.label
        scheduler = TaskScheduler(path_label, submit_cmd="sbatch")

        print(f"[Workflow] 正在构建 HONPAS 标注任务至: {path_label} ...")
        for i in range(len(candidates_sys)):
            scheduler.setup_task(i, wrapper, candidates_sys[i].data, psf_dir=cfg.TEMPLATE_DIR)
            
        print(f"✅ 任务构建完成。执行提交 (DryRun={dry_run})...")
        scheduler.submit_all(dry_run=dry_run)