...
*   **模型全生命周期管理**：提供实时的训练 Loss 监控与最终模型的冻结、压缩及精度评估以及gpumd主动学习迭代支持。
...
│       ├── monitor.py     # 训练监控
│       └── evaluator.py   # 模型评估
│   └── active_learning/   # [主动学习迭代模块]
│       ├── trainer.py     # 构建多模型系综训练环境
│       ├── explorer.py    # 自动化配置带主动学习判据的 MD 任务
│       └── selector.py    # 筛选高不确定度结构并备份为 npy 格式
...
```bash
python main.py --stage 8 --sub 1 --iter 1 --data_path data/training/merged_master_xxx
```

*   **输出**：`iter_01/01.train/`。在各子目录运行 `nep` 得到 4 个 `nep.txt`。

#### 8.2 准备 MD 探索
链接已训练模型，生成带有 `active` 关键字的 GPUMD `run.in`。
```bash
# --iter 指向迭代轮次，--data_path 指向 MD 初始结构
python main.py --stage 8 --sub 2 --iter 1 --data_path data/raw/model.xyz
```
*   **输出**： `iter_01/02.md/` (自动链接 01.train 里的模型并生成配置文件)

#### 8.3 候选结构提取
解析探索产生的 `active.xyz`，筛选高偏差构型送往 HONPAS 进行标注。
```bash
# --path 指向上步生成的训练根目录，--iter 指向迭代轮次
python main.py --stage 8 --sub 3 --iter 1
```
*   **输出**： `iter_01/03.select/` (npy数据)和 `iter_01/04.label/` (HONPAS任务)
---
...
###  主动学习 (`modules/active_learning`)
| 模块 | 职责 |
| :--- | :--- |
| **trainer.py** | **系综准备**：构建多模型系综 (Ensemble) 训练环境。 |
| **explorer.py** | **探索配置**：自动化生成带有 `active` 判据的 MD 任务。 |
| **selector.py** | **结构筛选**：解析 `active.xyz`，下采样高误差结构并备份为 npy。 |

###  核心控制
| 模块 | 职责 |
| :--- | :--- |
| **workflows.py** | **流程编排**：串联各子模块，定义 Stage 1~8 的标准化逻辑。 |
